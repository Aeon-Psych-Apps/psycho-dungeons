...
def main(input):
    ...
    for img_obj in image_resource:
        ...
        with Image.open(io.BytesIO(image_bytes)) as im:
            ...
            new_width = min(new_width, max_upscale_width)
            new_height = min(new_height, max_upscale_height)
            ...
            if getattr(im, 'is_animated', False) and im.format == 'GIF':
                ...
                original_loop = im.info.get('loop', 1)  # preserve original loop
            else:
                frames = [im.convert('RGBA').resize((new_width, new_height), Image.Resampling.NEAREST)]
                durations = None
                original_loop = 1
